...
### 3.复杂类型的注入
当一个类中有复杂类型的成员变量(如：数组、Map、propertiesh和List等)时，不易使用上述两种方式进行注入。<br>

**下面介绍复杂类型的注入方法**<br>

我们有如下复杂类型的类(注：set成员函数和构造函数省略)：<br>
```java
public class CollectionBean {
	private Object[] obj_arr;//数组类型注入
...
<beans xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns="http://www.springframework.org/schema/beans" xsi:schemaLocation="http://www.springframework.org/schema/beans http://www.springframework.org/schema/beans/spring-beans-4.2.xsd ">
	<!-- 复杂类型数组注入 -->
	<bean name="bean" class="com.noble.injection.CollectionBean">
		<property name="obj_arr">
			<array>
				<value>noble</value>
				<value>alex</value>
				<ref bean="user" /><!-- 注入一个类 -->
...
<!-- ============================================================ -->
		<bean name="house" class="com.noble.bean.Hose">
		<property name="addr" value="碧桂园"></property>
		<property name="size" value="180"></property>
	</bean>

	<bean name="user" class="com.noble.bean.User">
		<constructor-arg name="name" index="0" type="String" value="noble"></constructor-arg>
		<constructor-arg name="age" index="1" type="int" value="21"></constructor-arg>
		<constructor-arg name="house" index="2" type="com.noble.bean.Hose" ref="house"></constructor-arg>
	</bean>
...
#### 2.复杂List的注入方法
注入方法如下(仅仅将array改为list):<br>
```xml
<bean name="bean2" class="com.noble.injection.CollectionBean">
	<property name="list">
		<list>
			<value>noble</value>
			<value>alex</value>
...
注入方法如下：<br>
```xml
<bean name="bean3" class="com.noble.injection.CollectionBean">
	<property name="map">
		<map>
...
注入方法如下：<br>
```xml
<bean name="bean4" class="com.noble.injection.CollectionBean">
	<property name="prop">
		<props>
			<prop key="driverClass">com.mysql.jdbc.driver</prop>
			<prop key="username">noble</prop>
